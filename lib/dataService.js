...
export const dataService = {
  ...
  async updateSeriesImage(id, imageFile, oldImageUrl) {
    let imageUrl = '';
    ...
    if (isSupabaseConfigured && imageFile) {
      try {
        ...
        // Clean up old image from Supabase storage if it was inside Supabase
        if (oldImageUrl && oldImageUrl.includes('/storage/v1/object/public/series-images/')) {
          const oldFileName = oldImageUrl.split('/').pop();
          if (oldFileName) {
            await supabase.storage.from('series-images').remove([oldFileName]);
          }
        }
      } catch (e) {
        console.error("Supabase series image upload failed:", e);
        throw e;
      }
    } else {
      // Local development / fallback: read as Base64 data URL
      if (imageFile) {
    ...
    if (isSupabaseConfigured) {
      try {
        const { data, error } = await supabase
          ...
          .single();
        if (!error && data) return data;
        console.error("Supabase series update error:", error);
      } catch (e) {
        console.error("Failed to update series image in Supabase:", e);
      }
    }
    ...
    const updatedRecord = { ...target, image_url: imageUrl };
    saveSeries(updatedRecord);
